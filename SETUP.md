...
| 자동 테스트 | `supabase/tests/attendance.sql` **195개 검사** (A~L 구간) |
| 조직(부서·직급) | 부서·직급 배정, 집계를 부서로 걸러 봄. **직급은 권한이 아님** |
| 승인 라인 | 부서장이 자기 부서원의 근태 정정·재택·휴가를 승인. 부서장은 관리자가 지정 |
...
- 결과는 **예외 메시지**로 나옵니다: `TEST_RESULT: 통과 195 / 실패 0 ✅`
...
- [ ] **부서장**: 부서에 부서장을 지정 → 그 사람 계정으로 자기 부서원의 정정·재택·휴가가 보이고
      승인되는지, 다른 부서는 안 보이는지
