...
방향으로 전환 중이다. 이번 세션에 근태의 토대(기록 신뢰성 → 근무시간 산정 → 재택 승인 → 위치 인증)를 깔았다.
...
| 자동 테스트 | `supabase/tests/attendance.sql` 54개 검사 (근무시간·자정 넘김·위치·권한) |
...
> commute-battle 프로젝트. `SETUP.md` 읽고, 로드맵 1번(공휴일 달력 · 교대/유연근무제)부터 진행해줘.

다른 걸 먼저 하고 싶으면 "1번 대신 ○○ 먼저"라고 하면 됩니다(우선순위는 아래 로드맵 참고).
...
- **DB 마이그레이션은 0001~0010 전부 적용 완료**입니다. 지금 당장 실행할 SQL은 없습니다.
...
- 결과는 **예외 메시지**로 나옵니다: `TEST_RESULT: 통과 54 / 실패 0 OK`
...
- **2026-08-16·08-17에 도착 처리가 안 된 기록이 2건 남아 있습니다.** 0010에서 중복 차단 범위를
  '어제~오늘'로 넓혔기 때문에, 이 잔재가 남아 있는 동안은 새 출근이 막힐 수 있습니다.
  캘린더에서 정정 요청으로 도착 시각을 채워 넣거나, 지워도 되는 테스트 기록이면 말씀해 주세요.

---

## 🗺 로드맵 (근태 시스템, 법·제도 항목 제외)

1. **공휴일 달력** — 휴일이 토·일 고정이라 광복절 같은 날 일해도 휴일근로 가산이 안 잡힙니다.
   (자정 넘김은 0010에서 해결됨)
2. **교대/유연근무제** — 근무 스케줄이 아직 기기 localStorage에 있습니다. 서버로 올리면 `is_on_time`도
   서버 판정으로 바꿀 수 있습니다(지금은 경험치용 자기신고 값). 실제로 교대 근무를 쓸 때 하면 됩니다.
3. **월 마감(확정) 처리** — 지난달 기록이 계속 정정될 수 있는 상태입니다. 급여 정산 전 확정이 필요합니다.
4. 조직(부서·직급), 휴가 신청·잔여, 급여 시스템 연동 포맷.
5. 지오펜스 후속(필요해지면): 미인증 기록을 관리자가 화면에서 바로 승인/반려하는 흐름(지금은 근태
