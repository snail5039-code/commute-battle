...
| 자동 테스트 | `supabase/tests/attendance.sql` 85개 검사 (근무시간·자정 넘김·위치·공휴일·마감·권한) |
...
> commute-battle 프로젝트. `SETUP.md` 읽고, 로드맵 1번(교대/유연근무제)부터 진행해줘.
...
- **DB 마이그레이션은 0001~0012 전부 적용 완료**입니다. 지금 당장 실행할 SQL은 없습니다.
...
1. **교대/유연근무제** — 근무 스케줄이 아직 기기 localStorage에 있습니다. 서버로 올리면 `is_on_time`도
   서버 판정으로 바꿀 수 있습니다(지금은 경험치용 자기신고 값). 실제로 교대 근무를 쓸 때 하면 됩니다.
2. 조직(부서·직급), 휴가 신청·잔여, 급여 시스템 연동 포맷.
