...
| 자동 테스트 | `supabase/tests/attendance.sql` **126개 검사**. 휴가까지 한 파일에 들어왔다 |
...
1번은 회사에서 하면 '현재 위치' 버튼 한 번이면 끝납니다. 공휴일은 이미 22건 넣으셨습니다.

**제가 할 수 있는 것 (로드맵 순서)**

| # | 할 일 | 상태 |
|---|---|---|
| 1 | 조직(부서·직급) | 바로 가능. 부서별 집계·승인 라인의 전제 |
| 2 | 급여 시스템 연동 포맷 | ⛔ **막힘** — 어느 급여 시스템에 넣을지 알려주셔야 함 |
| 3 | 공휴일 매년 자동 갱신 | 바로 가능. 지금은 관리자가 매년 눌러야 함 |
| 4 | 지오펜스 후속(미인증 승인 흐름, 사업장 여러 곳) | 필요해지면 |
| ~~5~~ | ~~휴가 케이스를 회귀 테스트에 편입~~ | ✅ 2026-08-18 완료 (85 → 126개) |

**솔직한 의견**: 근태 시스템으로서 필요한 뼈대는 다 섰습니다. 다음 기능을 더 얹기보다
**실제로 한 달 써 보고 나온 문제를 잡는 게** 지금은 더 값어치 있습니다. 구성원이 1~2명인
상태에서 조직(부서·직급)을 만들어 봐야 쓸 데가 없습니다.
...
- 결과는 **예외 메시지**로 나옵니다: `TEST_RESULT: 통과 126 / 실패 0 ✅`
...
2. 급여 시스템 연동 포맷 — 어느 급여 시스템에 넣을지 정해지면 그 포맷에 맞춥니다.
3. 공휴일 후속: 매년 초 자동으로 다시 불러오기(지금은 관리자가 눌러야 함).
