...
import { Bell, Bot, BriefcaseBusiness, Cat, MapPinned, ShieldCheck, Trash2 } from 'lucide-react';
  ...
  return <div className="grid gap-5 lg:grid-cols-[14rem_minmax(0,1fr)]">
    ...
    <div className="min-w-0">
      {active === 'work' && <section id="work" aria-labelledby="work-title" className="card p-5 md:p-7"><h2 id="work-title" className="text-lg font-bold">근무 설정</h2><p className="mt-1 text-sm text-slate-500">기본 주소, 근무 시간과 요일별 근무 형태를 관리합니다.</p>
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold">집 주소<input value={props.homeAddress} onChange={(event) => props.onHomeAddressChange(event.target.value)} className="settings-control mt-2 w-full rounded-xl border border-slate-200 px-3.5 text-sm" autoComplete="street-address"/></label><label className="text-xs font-bold">회사 주소<input value={props.workAddress} onChange={(event) => props.onWorkAddressChange(event.target.value)} className="settings-control mt-2 w-full rounded-xl border border-slate-200 px-3.5 text-sm" autoComplete="street-address"/></label><label className="text-xs font-bold">출근 시각<input type="time" value={props.schedule.startTime} onChange={(event) => props.onScheduleChange({ ...props.schedule, startTime: event.target.value })} className="settings-control mt-2 w-full rounded-xl border border-slate-200 px-3"/></label><label className="text-xs font-bold">퇴근 시각<input type="time" value={props.schedule.endTime} onChange={(event) => props.onScheduleChange({ ...props.schedule, endTime: event.target.value })} className="settings-control mt-2 w-full rounded-xl border border-slate-200 px-3"/></label></div>
      ...
      {active === 'account' && <section id="account" aria-labelledby="account-title" className="card p-5 md:p-7"><h2 id="account-title" className="text-lg font-bold">계정</h2><div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4"><div className="flex items-start gap-3"><Trash2 className="mt-0.5 shrink-0 text-red-600" size={18}/><div><h3 className="text-sm font-bold text-red-800">계정 삭제</h3><p className="mt-1 text-xs leading-5 text-red-700">현재 앱에서는 계정 삭제를 직접 실행하지 않습니다. 삭제가 필요하면 서비스 관리자에게 계정 ID와 함께 요청하세요. 이 화면의 버튼이나 설정 초기화로 계정이 삭제되지는 않습니다.</p></div></div></div></section>}
