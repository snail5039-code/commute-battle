...
import { STAGE_NAMES, type CharacterStage } from '@/lib/characterStages';

export default function EvolutionCelebration({ level, stage, evolved, onClose }: { level: number; stage: CharacterStage; evolved: boolean; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-label={evolved ? '진화 축하' : '레벨업 축하'}>
    <div className="relative w-full max-w-sm overflow-hidden rounded-3xl bg-white p-8 text-center shadow-2xl motion-safe:animate-[celebrate-in_.45s_ease-out]">
      ...
      <div className="mx-auto grid size-20 place-items-center rounded-full bg-amber-100 text-amber-600 motion-safe:animate-[celebrate-bounce_.8s_ease-in-out_infinite]"><Sparkles size={38} /></div>
