      ...
      <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5">
        ...
        <section className="card p-5 sm:col-span-2" aria-label="퇴근 경로 요약">
          ...
          {!returnRoute?.origin || !returnRoute.destination ? <div className="mt-4 rounded-xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">저장된 회사·집 주소가 모두 있어야 퇴근 경로를 요약할 수 있어요. 설정에서 주소를 등록해 주세요.</div> : <div className="mt-4 grid gap-3 md:grid-cols-[1fr_1.4fr]">
            ...
            {returnRoute.route ? <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4"><p className="text-xs text-slate-500">최근 선택한 퇴근 경로</p><p className="mt-1 truncate text-sm font-bold" title={returnRoute.route.signature}>{returnRoute.route.signature}</p><p className="mt-2 text-xs text-slate-600">예상 {Math.round(returnRoute.route.totalTime)}분 · 도보 {Math.round(returnRoute.route.totalWalk)}m · 환승 {returnRoute.route.transferCount}회</p></div> : <div className="rounded-xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">아직 저장된 퇴근 경로 기록이 없어요. 퇴근 경로를 한 번 선택하면 최근 예상 정보가 여기에 표시됩니다.</div>}
          </div>}
        </section>
