...
export default function DepartmentChat() {
  ...
  return <div className="grid h-[calc(100dvh-4.5rem)] min-h-[38rem] overflow-hidden bg-white md:h-screen lg:grid-cols-[16.5rem_minmax(0,1fr)]">
    <aside className="flex min-h-0 flex-col border-b border-[var(--nav-border)] bg-[var(--nav-bg)] text-[var(--nav-text)] lg:border-b-0 lg:border-r">
      ...
      <div className="border-b border-white/10 p-2.5"><label className="flex h-8 items-center gap-2 border border-white/25 bg-[#260027]/40 px-2 text-white/60 focus-within:border-white/70 focus-within:text-white"><Search size={14}/><span className="sr-only">채널 검색</span><input value={channelQuery} onChange={(event) => setChannelQuery(event.target.value)} placeholder="채널 검색" className="min-w-0 flex-1 bg-transparent text-xs text-white outline-none placeholder:text-white/50"/></label></div>
