...
import { getExpNeeded, STAGE_NAMES } from '@/lib/characterStages';
...
export default function BadgesWidget({ records, user }: { records: CommuteRecord[]; user?: User }) {
  ...
  return <section className="card h-full overflow-hidden" aria-labelledby="badge-widget-title">
    {user && <div className="border-b border-slate-100 bg-gradient-to-r from-blue-50/80 to-indigo-50/60 px-5 py-4"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-white text-blue-600 shadow-sm ring-1 ring-blue-100"><CharacterIcon stage={user.character_stage} size={21} /></div><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><p className="truncate text-xs font-bold text-slate-800">{STAGE_NAMES[user.character_stage]} · Lv.{user.character_level}</p><span className="text-[10px] font-semibold text-blue-700">배지 {completed}개</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-blue-500" style={{ width: `${expPercent}%` }} /></div><p className="mt-1 text-[10px] text-slate-500">EXP {user.character_exp} / {expNeeded}</p></div></div></div>}
