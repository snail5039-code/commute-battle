...
export default function BadgesPage() {
  ...
  return <div className="flex min-h-screen flex-col"><TopBar title="배지와 퀘스트" subtitle={`나의 배지 ${completed} / ${total}`} />
    <main className="flex-1 p-4 md:p-8"><div className="mx-auto max-w-5xl space-y-6">
      <section className="card overflow-hidden bg-gradient-to-br from-slate-900 to-blue-950 p-6 text-white"><div className="flex min-w-0 flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="min-w-0"><div className="mb-2 flex items-center gap-2 text-blue-200"><Sparkles size={16} /><span className="text-xs font-bold tracking-widest">나의 배지 모음</span></div><h1 className="break-keep text-2xl font-black">출퇴근 모험 도감</h1><p className="mt-2 max-w-lg break-words text-sm leading-relaxed text-slate-300">실제 출퇴근 기록으로 배지와 펫 액세서리를 해금하세요. 휴가·병가는 연속 출근을 보호하고 결근은 기록을 끊습니다.</p></div><div className="shrink-0 text-right"><strong className="text-3xl">{completed}</strong><span className="text-slate-400"> / {total}</span></div></div></section>
