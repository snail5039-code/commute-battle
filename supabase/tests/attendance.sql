...
begin
  ...
  checks := checks + 5;
  if (d->>'workedMinutes')::numeric <> 480 then fails := fails || format('A-1 근무: 기대=480 실제=%s', d->>'workedMinutes'); end if;
  if (d->>'breakMinutes')::numeric <> 60 then fails := fails || format('A-1 휴게: 기대=60 실제=%s', d->>'breakMinutes'); end if;
  if (d->>'overtimeMinutes')::numeric <> 0 then fails := fails || format('A-1 연장: 기대=0 실제=%s', d->>'overtimeMinutes'); end if;
  if (d->>'nightMinutes')::numeric <> 0 then fails := fails || format('A-1 야간: 기대=0 실제=%s', d->>'nightMinutes'); end if;
  if d->>'status' <> 'complete' then fails := fails || format('A-1 상태: 기대=complete 실제=%s', d->>'status'); end if;
  ...
  checks := checks + 4;
  if (d->>'workedMinutes')::numeric <> 750 then fails := fails || format('A-2 근무: 기대=750 실제=%s', d->>'workedMinutes'); end if;
  if (d->>'overtimeMinutes')::numeric <> 270 then fails := fails || format('A-2 연장: 기대=270 실제=%s', d->>'overtimeMinutes'); end if;
  if (d->>'nightMinutes')::numeric <> 60 then fails := fails || format('A-2 야간: 기대=60 실제=%s', d->>'nightMinutes'); end if;
  if (d->>'lateMinutes')::numeric <> 30 then fails := fails || format('A-2 지각: 기대=30 실제=%s', d->>'lateMinutes'); end if;
  ...
  checks := checks + 5;
  if (d->>'workedMinutes')::numeric <> 270 then fails := fails || format('A-3 근무: 기대=270 실제=%s', d->>'workedMinutes'); end if;
  if (d->>'breakMinutes')::numeric <> 30 then fails := fails || format('A-3 휴게: 기대=30 실제=%s', d->>'breakMinutes'); end if;
  if (d->>'holidayMinutes')::numeric <> 270 then fails := fails || format('A-3 휴일: 기대=270 실제=%s', d->>'holidayMinutes'); end if;
  if (d->>'lateMinutes')::numeric <> 0 then fails := fails || format('A-3 지각(휴일엔 미적용): 기대=0 실제=%s', d->>'lateMinutes'); end if;
  if (d->>'isHoliday')::boolean is not true then fails := fails || 'A-3 휴일플래그: 기대=true'; end if;

  -- A-4. 휴가
  select item into d from jsonb_array_elements(summary->'days') item where item->>'date' = d5::text;
  checks := checks + 2;
  if d->>'status' <> 'vacation' then fails := fails || format('A-4 상태: 기대=vacation 실제=%s', d->>'status'); end if;
  if (d->>'workedMinutes')::numeric <> 0 then fails := fails || format('A-4 근무: 기대=0 실제=%s', d->>'workedMinutes'); end if;
  ...
  checks := checks + 6;
  if d is null then
    fails := fails || 'B-1 수요일 행이 없음';
  else
    -- 09:30 → 익일 01:00 = 930분, 휴게 60 차감 = 870분
    if (d->>'workedMinutes')::numeric <> 870 then fails := fails || format('B-1 근무: 기대=870 실제=%s', d->>'workedMinutes'); end if;
    if (d->>'overtimeMinutes')::numeric <> 390 then fails := fails || format('B-2 연장: 기대=390 실제=%s', d->>'overtimeMinutes'); end if;
    -- 야간은 22:00~익일 01:00 = 180분
    if (d->>'nightMinutes')::numeric <> 180 then fails := fails || format('B-3 야간: 기대=180 실제=%s', d->>'nightMinutes'); end if;
    if (d->>'lateMinutes')::numeric <> 30 then fails := fails || format('B-4 지각: 기대=30 실제=%s', d->>'lateMinutes'); end if;
    -- 퇴근이 소정근로 종료(18:00)보다 늦으므로 조기퇴근은 0이어야 한다(음수가 새면 안 됨)
    if (d->>'earlyOutMinutes')::numeric <> 0 then fails := fails || format('B-5 조기퇴근: 기대=0 실제=%s', d->>'earlyOutMinutes'); end if;
    if d->>'status' <> 'complete' then fails := fails || format('B-6 상태: 기대=complete 실제=%s', d->>'status'); end if;
  end if;
  ...
  if exists (select 1 from jsonb_array_elements(summary->'days') item where item->>'date' = d4::text) then
    fails := fails || 'B-7 목요일에 행이 생김 — 퇴근이 다음 날로 갈라졌다';
  end if;
  ...
  checks := checks + 3;
  if (w->>'workedMinutes')::numeric <> 2370 then fails := fails || format('B-8 주간근무: 기대=2370 실제=%s', w->>'workedMinutes'); end if;
  if (w->>'overtimeMinutes')::numeric <> 660 then fails := fails || format('B-8 주간연장: 기대=660 실제=%s', w->>'overtimeMinutes'); end if;
  if (w->>'overLimit')::boolean is not false then fails := fails || 'B-8 주52시간: 기대=false'; end if;
  ...
  if public.attendance_work_date(uid::text, now()) <> today - 1 then
    fails := fails || format('C-2 어제 미완결: 기대=%s 실제=%s', today - 1, public.attendance_work_date(uid::text, now()));
  end if;
  ...
  checks := checks + 1;
  if public.attendance_work_date(uid::text, now()) <> today then
    fails := fails || format('C-3 어제 닫힘: 기대=%s 실제=%s', today, public.attendance_work_date(uid::text, now()));
  end if;
  ...
  checks := checks + 1;
  if public.attendance_work_date(uid::text, now()) <> today then
    fails := fails || format('C-4 둘 다 열림: 기대=%s 실제=%s', today, public.attendance_work_date(uid::text, now()));
  end if;
  ...
  checks := checks + 1;
  if public.attendance_work_date(uid::text, now()) <> today then
    fails := fails || format('C-5 사흘 전 잔재: 기대=%s 실제=%s', today, public.attendance_work_date(uid::text, now()));
  end if;
  ...
  checks := checks + 3;
  if rec.date <> today then fails := fails || format('D-1 출근 근무일: 기대=%s 실제=%s', today, rec.date); end if;
  if rec.start_time is null then fails := fails || 'D-1 출발 시각이 비어 있음'; end if;
  if rec.end_time is not null then fails := fails || 'D-1 출발 직후엔 도착 시각이 없어야 함'; end if;

  -- D-2. 도착 처리 전에는 새 출발을 막는다
  checks := checks + 1;
  begin
    perform public.attendance_start('return', ws);
    fails := fails || 'D-2 중복 차단: 예외가 나야 하는데 통과됨';
  exception when others then
    if sqlerrm not like '%도착 처리되지 않은%' then
      fails := fails || format('D-2 중복 차단: 예상과 다른 오류 %s', sqlerrm);
    end if;
  end;
  ...
  checks := checks + 2;
  if rec.end_time is null then fails := fails || 'D-3 도착 시각이 안 채워짐'; end if;
  if rec.duration_minutes is null then fails := fails || 'D-3 이동 시간이 안 채워짐'; end if;
  ...
  begin
    rec := public.attendance_start('commute', ws);
  exception when others then
    fails := fails || format('D-4 사흘 전 잔재가 오늘 출근을 막음: %s', sqlerrm);
  end;
  ...
  checks := checks + 2;
  if loc.verified is not true then fails := fails || format('E-1 반경 안: 기대=true 실제=%s', loc.verified); end if;
  if loc.status <> 'verified' then fails := fails || format('E-1 상태: 기대=verified 실제=%s', loc.status); end if;

  -- E-2. 반경 밖 (약 1.1km)
  select * into loc from public.attendance_location_check(ws, 37.576535, 126.977969, 20, false);
  checks := checks + 3;
  if loc.verified is not false then fails := fails || format('E-2 반경 밖: 기대=false 실제=%s', loc.verified); end if;
  if loc.status <> 'out_of_range' then fails := fails || format('E-2 상태: 기대=out_of_range 실제=%s', loc.status); end if;
  if loc.distance_m < 1000 or loc.distance_m > 1200 then fails := fails || format('E-2 거리: 기대=약 1.1km 실제=%s', loc.distance_m); end if;
  ...
  checks := checks + 2;
  if loc.verified is not false then fails := fails || format('E-3 정확도 부족: 기대=false 실제=%s', loc.verified); end if;
  if loc.status <> 'low_accuracy' then fails := fails || format('E-3 상태: 기대=low_accuracy 실제=%s', loc.status); end if;

  -- E-4. 위치 권한 거부 / 신호 없음
  select * into loc from public.attendance_location_check(ws, null, null, null, true);
  checks := checks + 1;
  if loc.status <> 'denied' then fails := fails || format('E-4 권한 거부: 기대=denied 실제=%s', loc.status); end if;
  select * into loc from public.attendance_location_check(ws, null, null, null, false);
  checks := checks + 1;
  if loc.status <> 'unavailable' then fails := fails || format('E-4 신호 없음: 기대=unavailable 실제=%s', loc.status); end if;
  ...
  checks := checks + 2;
  if loc.verified is not null then fails := fails || format('E-5 좌표 미설정: 기대=null 실제=%s', loc.verified); end if;
  if loc.status <> 'no_policy' then fails := fails || format('E-5 상태: 기대=no_policy 실제=%s', loc.status); end if;

  -- E-6. 워크스페이스가 없는 개인 기록도 검증 대상이 아니다
  select * into loc from public.attendance_location_check(null, 37.400000, 127.100000, 10, false);
  checks := checks + 1;
  if loc.verified is not null then fails := fails || format('E-6 개인 기록: 기대=null 실제=%s', loc.verified); end if;
  ...
  if has_function_privilege('authenticated', 'public.attendance_location_check(uuid, double precision, double precision, numeric, boolean)', 'execute')
    then fails := fails || 'F-1 attendance_location_check가 authenticated에게 열려 있음'; end if;
  if has_function_privilege('anon', 'public.attendance_location_check(uuid, double precision, double precision, numeric, boolean)', 'execute')
    then fails := fails || 'F-2 attendance_location_check가 anon에게 열려 있음'; end if;
  if has_function_privilege('authenticated', 'public.geo_distance_m(double precision, double precision, double precision, double precision)', 'execute')
    then fails := fails || 'F-3 geo_distance_m이 authenticated에게 열려 있음'; end if;
  if has_function_privilege('anon', 'public.get_attendance_summary(uuid, date, date, text)', 'execute')
    then fails := fails || 'F-4 get_attendance_summary가 anon에게 열려 있음'; end if;
