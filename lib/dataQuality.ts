...
export const QUALITY_REASON_LABELS: Record<DataQualityReason, string> = {
  duplicate: '중복 기록', incomplete: '미완료 기록', invalid_duration: '비정상 이동시간', gps_jump: 'GPS 이상',
};
...
function numberField(record: CommuteRecord, names: string[]) {
  ...
  for (const name of names) { const value = Number(raw[name]); if (Number.isFinite(value)) return value; }
  return null;
}
...
function completedTripRequired(record: CommuteRecord) { return record.type === 'commute' || record.type === 'return'; }
...
export function assessDataQuality(records: CommuteRecord[]): QualityResult {
  ...
  for (const record of records) {
    ...
    const key = `${record.user_id}|${record.date}|${record.type}`;
    if (seen.has(key)) reasons.push('duplicate'); else seen.add(key);
    if (completedTripRequired(record) && (!record.start_time || !record.end_time)) reasons.push('incomplete');
    if (completedTripRequired(record) && (typeof record.duration_minutes !== 'number' || !Number.isFinite(record.duration_minutes) || record.duration_minutes < 0 || record.duration_minutes > MAX_TRIP_MINUTES)) reasons.push('invalid_duration');
    if (hasGpsJump(record)) reasons.push('gps_jump');
    if (reasons.length) { reasons.forEach((reason) => counts[reason]++); excludedRecords.push({ record, reasons }); } else validRecords.push(record);
  }
  return { validRecords, excludedRecords, counts };
}
export function qualitySummary(result: QualityResult) {
  return (Object.entries(result.counts) as [DataQualityReason, number][]).filter(([, count]) => count > 0).map(([reason, count]) => `${QUALITY_REASON_LABELS[reason]} ${count}건`);
}
