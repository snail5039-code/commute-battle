...
export interface QualityResult {
  validRecords: CommuteRecord[];
  excludedRecords: QualityIssue[];
  counts: Record<DataQualityReason, number>;
}

export const QUALITY_REASON_LABELS: Record<DataQualityReason, string> = {
  duplicate: '중복 기록',
  incomplete: '미완료 기록',
  invalid_duration: '음수·비정상 이동시간',
  gps_jump: 'GPS 점프',
};

const MAX_TRIP_MINUTES = 4 * 60;
...
function hasGpsJump(record: CommuteRecord) {
  ...
  const distanceKm = numberField(record, ['distance_km']);
  const distanceM = numberField(record, ['distance_meters', 'distance']);
  const kilometers = distanceKm ?? (distanceM === null ? null : distanceM / 1000);
  const duration = record.duration_minutes;
  return kilometers !== null && typeof duration === 'number' && duration > 0
    ? kilometers / (duration / 60) > MAX_REASONABLE_SPEED_KMH
    : false;
}

function duplicateKey(record: CommuteRecord) {
  return [record.user_id, record.date, record.type].join('|');
}

function requiresCompletedTrip(record: CommuteRecord) {
...
export function assessDataQuality(records: CommuteRecord[]): QualityResult {
  ...
  for (const record of records) {
    ...
    if (requiresCompletedTrip(record) && (!record.start_time || !record.end_time)) reasons.push('incomplete');
    if (
      requiresCompletedTrip(record) &&
      (typeof record.duration_minutes !== 'number' ||
        !Number.isFinite(record.duration_minutes) ||
        record.duration_minutes < 0 ||
        record.duration_minutes > MAX_TRIP_MINUTES)
